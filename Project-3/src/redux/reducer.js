...
const reducer = (state = initialState, action) => {
    switch(action.type){
        case 'FILTER_POKEMON_BY_TYPE':
            let filter = action.payload === 'All' ?
            state.pokemons : state.pokemons.filter((pokemon) => pokemon.types.includes(action.payload));
            console.log(filter);
            return{...state, filteredPokemon: filter};
        case 'SHOW_DESCRIPTION':
            let selected = state.pokemons.find(pokemon => pokemon.name === action.payload)
            console.log(selected);
            return{...state, selectedPokemon: selected};
        default:
            state.filteredPokemon = state.pokemons;
            return (state);
    }
} 
